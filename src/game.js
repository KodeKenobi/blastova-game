...
import { BarrelCalibrator } from './systems/barrel-calibrator-system';
import {
...
const GameScene = new Phaser.Class({
  ...
  create: function () {
    ...
    initOnboarding();
    this.barrelCalibrator = new BarrelCalibrator(this);
    return result;
  },
  ...
  setupWeaponFireTestRig: function () {
    ...
    const testWeaponId = DEBUG_FLAGS.tower1TestWeaponId || 1;
    const testWeapon = (this.towerCatalog || []).find(w => w.id === testWeaponId) || this.towerCatalog?.[0] || null;
    if (testWeapon) {
      this.gameState.gold = Math.max(this.gameState.gold || 0, 9999);
      this.selectTowerDef?.(testWeapon);
      ...
      for (let i = 0; i < placementAttempts.length; i += 1) {
        ...
        if (this.tryPlaceTower(attempt.x, attempt.y, testWeapon)) {
          placedTower = this.towers?.children?.entries?.find((tower) => tower?.active && tower.getData('towerId') === testWeapon.id) || null;
          if (placedTower) {
            break;
          }
        }
      ...
      if (placedTower) {
        ...
        if (testDummy) {
          const towerRange = Math.max(80, Number(placedTower.getData('range') || testWeapon.range || 170));
        ...
        this.setStatus('Test mode: could not auto-place ' + testWeapon.name, '#ffb18b');
      }
    }

    this.setStatus('Weapon fire test mode active. ' + (testWeapon?.name || 'Weapon') + ' is firing continuously.', '#9df8ff');
  },
  ...
  updateTowerFire: function () {
    ...
    this.towers.children.entries.forEach((tower) => {
      ...
      // In fire test mode, fire continuously without rotating
      if (DEBUG_FLAGS.fireTestMode) {
        const lastFire = tower.getData('lastFire') || 0;
        const fireRate = tower.getData('fireRate') || 800;
        ...
        this.fireTowerWeapon(tower, dummyTarget);
  ...
  playShotSfx: function (vfxProfile, durationMs = 70, peakGain = 0.045) {
    ...
    // If buffered audio played, don't play synth
    if (playedBufferedSample) {
      return;
    }
    // Only play synth if buffered audio NOT available
    if (nowMs - (this.lastSynthShotAt || -9999) < synthCadenceGateMs) {
      return;
    }
    ...
    try {
      ...
      // BOOST synth when no buffered audio — make it THICK and LOUD
      const synthPeakGain = playedBufferedSample
        ? Phaser.Math.Clamp(peakGain * 0.22, 0.004, 0.014)
        : Phaser.Math.Clamp(peakGain * 8.8, 0.4, 0.65); // VERY LOUD synth for ballistics
      const busAdjustedPeakGain = synthPeakGain * Math.max(weaponBusVolume, 0.8); // Ensure at least 80% volume
      ...
      lowpass.frequency.setValueAtTime(3800, now); // Higher cutoff for thicker bass
      lowpass.Q.setValueAtTime(2.2, now); // Higher resonance for meatier sound

      gain.gain.setValueAtTime(0.0001, now);
      gain.gain.exponentialRampToValueAtTime(busAdjustedPeakGain, now + 0.008); // Faster attack for punch
  ...
  applyTowerRecoil: function (tower, angleRad, vfxProfile) {
    if (!tower?.active || DEBUG_FLAGS.fireTestMode) {
      return;
    }
  ...
  spawnRocketMuzzle: function (tower, angleRad, customX = null, customY = null) {
    const distance = sx(20);
    const fx = (typeof customX === 'number') ? customX : (tower.x + Math.cos(angleRad) * distance);
    const fy = (typeof customY === 'number') ? customY : (tower.y + Math.sin(angleRad) * distance);
  ...
  fireRocketSalvo: function (tower, target) {
    ...
    const allBarrelOffsets = tower.getData('barrelOffsets');
    // If we have lateral 0 center barrel, filter outer barrels for rocket salvo if there are outer barrels
    const barrelOffsets = (Array.isArray(allBarrelOffsets) && allBarrelOffsets.some(b => (b.lateral ?? b.x ?? 0) === 0))
      ? allBarrelOffsets.filter(b => (b.lateral ?? b.x ?? 0) !== 0)
      : allBarrelOffsets;

    const rocketCount = Math.max(1, (Array.isArray(barrelOffsets) && barrelOffsets.length > 0) ? barrelOffsets.length : (tower.getData('rocketCount') || 6));
    const interval = Number.isFinite(tower.getData('rocketInterval')) ? Number(tower.getData('rocketInterval')) : 0;
    const spread = tower.getData('rocketSpread') ?? 16;
    if (!DEBUG_FLAGS.fireTestMode) {
      this.triggerTowerFireAnimation(tower, (rocketCount * interval) + 220);
    }

    const spawnSingleRocket = (i) => {
      if (!tower.active) {
        return;
      }

      let dynamicTarget = (target && target.active) ? target : this.selectEnemyForTower(tower);
      if (!dynamicTarget) {
        return;
      }
      ...
      const specificBarrel = (Array.isArray(barrelOffsets) && barrelOffsets.length > 0) ? barrelOffsets[i % barrelOffsets.length] : null;
      const projectile = this.fireProjectile(tower, dynamicTarget, angleOffset, {
        suppressDefaultFx: true,
        projectileKey: 'rocketProjectile',
        barrelOffset: specificBarrel,
        barrelSlot: tower.getData('barrelSlotLayout')?.[i] ?? i,
        barrelCount: tower.getData('barrelLayoutCount') || rocketCount,
        barrelSpacing: tower.getData('barrelSpacing') ?? undefined,
        barrelLateralOffset: specificBarrel ? (specificBarrel.lateral ?? specificBarrel.x) : tower.getData('barrelLateralOffsets')?.[i],
        barrelForwardOffset: specificBarrel ? (specificBarrel.forward ?? specificBarrel.y) : tower.getData('barrelForwardOffset'),
      });
      if (!projectile) {
        return;
      }
      ...
      this.spawnRocketMuzzle(tower, angleRad, projectile.x, projectile.y);
      ...
      this.spawnShotTracer(
        projectile.x,
        projectile.y,
        projectile.x + Math.cos(angleRad) * sx(70),
        projectile.y + Math.sin(angleRad) * sx(70),
        0xffb670,
        sx(2.5),
        0.62,
        105
    ...
    for (let i = 0; i < rocketCount; i++) {
      if (interval > 0) {
        this.time.delayedCall(i * interval, () => spawnSingleRocket(i));
      } else {
        spawnSingleRocket(i);
      }
    }
  ...
  fireProjectile: function (tower, target, angleOffset = 0, options = {}) {
    ...
    const baseAimRad = Phaser.Math.Angle.Between(tower.x, tower.y, target.x, target.y);
    const baseAimDeg = Phaser.Math.RadToDeg(baseAimRad);
    const angle = (Number.isFinite(baseAimDeg) ? baseAimDeg : 0) + angleOffset;
    const angleRad = Phaser.Math.DegToRad(angle);
    const turretAngleRad = Number.isFinite(baseAimRad) ? baseAimRad : 0;

    let spawnX = tower.x;
    let spawnY = tower.y;
    const towerId = tower.getData('towerId') || 0;
    const isDualBarrelBurst = (tower.getData('firePattern') === 'burst') && (towerId === 1 || towerId === 2);
    const towerScaleX = tower.scaleX || 1.26;
    const towerScaleY = tower.scaleY || 1.26;

    if (options.barrelOffset || (Number.isFinite(options.barrelLateralOffset) && Number.isFinite(options.barrelForwardOffset))) {
      const latVal = options.barrelOffset ? (options.barrelOffset.lateral ?? options.barrelOffset.x ?? 0) : options.barrelLateralOffset;
      const fwdVal = options.barrelOffset ? (options.barrelOffset.forward ?? options.barrelOffset.y ?? 0) : options.barrelForwardOffset;
      const forward = fwdVal * towerScaleY;
      const lateral = latVal * towerScaleX;
      spawnX += (Math.cos(turretAngleRad) * forward) + (-Math.sin(turretAngleRad) * lateral);
      spawnY += (Math.sin(turretAngleRad) * forward) + (Math.cos(turretAngleRad) * lateral);
    } else if (isDualBarrelBurst) {
      ...
      const forward = sx(options.barrelForwardOffset ?? 14);
      const lateral = Number.isFinite(options.barrelLateralOffset)
        ? sx(options.barrelLateralOffset)
        : sx(options.barrelSpacing || 5.8) * lateralUnits;
      spawnX += (Math.cos(turretAngleRad) * forward) + (-Math.sin(turretAngleRad) * lateral);
      spawnY += (Math.sin(turretAngleRad) * forward) + (Math.cos(turretAngleRad) * lateral);
    }

    const projectile = this.projectiles.create(spawnX, spawnY, projectileKey);
    projectile.setDepth(10); // Always render above map, enemies, and towers
    const baseScale = (tower.getData('projectileScale') || 1.15) * (vfxProfile.projectileScaleMult || 1);
    projectile.setScale(baseScale * 1.6); // 60% thicker projectiles
    ...
    root.innerHTML = `
      <style>
        #upgrade-modal-root .um-panel {
          background: linear-gradient(180deg,rgba(23,20,16,0.97),rgba(14,12,10,0.98));
          border: 2px solid rgba(150,138,84,0.55);
          border-radius: 16px;
          box-shadow: 0 0 0 1px rgba(150,138,84,0.18) inset, 0 12px 48px rgba(0,0,0,0.7);
        ...
        #upgrade-modal-root .um-art {
          ...
          min-width: 100px;
          height: 100px;
          border: 1px solid rgba(150,138,84,0.3);
          border-radius: 10px;
          background: rgba(15,14,10,0.85) no-repeat center/contain;
          display: flex;
          align-items: center;
          justify-content: center;
          color: rgba(200,190,150,0.35);
          font-size: 11px;
          text-align: center;
        ...
        #upgrade-modal-root .um-stat-label { color: rgba(200,190,150,0.65); }
        #upgrade-modal-root .um-bar-wrap {
          height: 5px;
          background: rgba(150,138,84,0.12);
          border-radius: 3px;
          overflow: hidden;
        }
        #upgrade-modal-root .um-bar-fill {
          height: 100%;
          border-radius: 3px;
          background: linear-gradient(90deg,#8a7233,#e8c97a);
          transition: width 0.3s;
        }
        #upgrade-modal-root .um-bar-fill.gain { background: linear-gradient(90deg,#3adf80,#9fffd4); }
        #upgrade-modal-root .um-stat-val { color: #ece4c8; font-weight: 600; text-align: right; min-width: 36px; }
        ...
        #upgrade-modal-root .um-cost-row {
          ...
          border: 1px solid rgba(150,138,84,0.2);
          border-radius: 8px;
          padding: 9px 14px;
          margin-bottom: 12px;
        }
        #upgrade-modal-root .um-cost-label { font-size: 12px; color: rgba(200,190,150,0.6); font-family:"Teko",sans-serif; text-transform:uppercase; letter-spacing:0.8px; }
        ...
        #upgrade-modal-root .um-nav-btn {
          background: rgba(20,19,13,0.9);
          border: 1px solid rgba(150,138,84,0.35);
          color: #e8dbc3;
          font-size: 18px;
        ...
        #upgrade-modal-root .um-nav-btn:hover { background: rgba(150,138,84,0.15); }
        #upgrade-modal-root .um-counter { font-size: 12px; color: rgba(210,200,165,0.5); font-family:"Teko",sans-serif; min-width:40px; text-align:center; }
        ...
        #upgrade-modal-root .um-btn-upgrade.can-afford { background:#15160f; border-color:#3adf80; color:#9fffd4; }
        #upgrade-modal-root .um-btn-upgrade.can-afford:hover { background: rgba(58,223,128,0.12); }
        #upgrade-modal-root .um-btn-upgrade.cant-afford { background:#15160f; border-color:#882222; color:#ff9090; cursor:not-allowed; opacity:0.7; }
        #upgrade-modal-root .um-btn-skip {
          flex: 1;
          min-height: 48px;
          padding: 11px 18px; border-radius: 8px; font-size: 12px; font-weight: 700;
          cursor: pointer; background: #15160f; border: 1px solid rgba(160,146,91,0.35);
          color: #e7e0c5; text-transform: uppercase; letter-spacing: 1px;
          font-family: 'BlackOpsOne',"Teko",sans-serif;
        }
        #upgrade-modal-root .um-btn-skip:hover { background: rgba(150,138,84,0.15); }
        #upgrade-modal-root .um-btn-armory {
          ...
          font-size: 12px;
          font-weight: 700;
          cursor: pointer; background: #15160f; border: 1px solid rgba(255,180,40,0.4);
          color: #ffd97a; text-transform: uppercase; letter-spacing: 1px;
          font-family: 'BlackOpsOne',"Teko",sans-serif;
        }
        #upgrade-modal-root .um-btn-armory:hover { background: rgba(255,180,40,0.15); }
        @keyframes um-flash {
...
// Attempt to lock screen orientation to landscape where the platform allows it
// (PWA installed / fullscreen contexts). Native Android/iOS builds enforce
// landscape via their manifest/Info.plist configuration instead.
const tryLockLandscapeOrientation = async () => {
  try {
    ...
    // Orientation lock requires fullscreen/installed-PWA context in most browsers;
    // silently ignore when unsupported so the CSS rotate-overlay handles the rest.
  }
};
tryLockLandscapeOrientation();
document.addEventListener('fullscreenchange', tryLockLandscapeOrientation);
